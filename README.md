...
- [탐지 엔진 상세](#탐지-엔진-상세)
- [NetFlow 기반 탐지 엔진](#netflow-기반-탐지-엔진-span-불필요)
- [웹 대시보드](#웹-대시보드)
- [스크린샷](#스크린샷)
- [빠른 시작](#빠른-시작)
- [설정 가이드](#설정-가이드)
...
**Panopticon**은 소규모~중규모 네트워크 환경을 위한 올인원 네트워크 보안 모니터링 시스템이다. Scapy 기반의 실시간 패킷 캡처부터 21개의 패킷 기반 탐지 엔진 + 2개의 NetFlow 기반 탐지 엔진, 킬체인 기반 인시던트 상관분석, 자동 IP 차단(IRS), 위협 인텔리전스 피드 통합, AI 기반 오탐 자동 감소(AIAnalyzer), 그리고 실시간 웹 대시보드까지 — 네트워크 보안 모니터링에 필요한 모든 것을 단일 바이너리로 제공한다.
...
### 21개 패킷 기반 탐지 엔진 + 2개 NetFlow 기반 탐지 엔진

패킷 계층(L2~L7), 통계 이상, 행위 프로파일, 위협 인텔리전스, 사용자 정의 시그니처까지 네트워크 보안의 전 영역을 커버한다. 각 엔진은 독립적으로 활성화/비활성화 및 파라미터 조정이 가능하다. SPAN이 없는 환경에서는 NetFlow 기반 엔진 2개(`flow_port_scan`, `flow_data_exfil`)로 라우터 수준 가시성을 보완한다.
...
Chart.js 기반 트래픽 시각화, WebSocket 실시간 이벤트 스트리밍, 장치 관리, 엔진 설정, 차단 목록·허용 목록 관리를 통합한 단일 페이지 대시보드를 제공한다. 프론트엔드는 ES 모듈 기반으로 분리되어 있으며 한국어/영어 i18n을 지원한다.
...
| C2 Beaconing (`c2_beaconing`) | | O |
| Segment Violation (`segment_violation`) | | O |
| Dark IP (`dark_ip`) | | O |
...
- 출발지 IP 일치: 알려진 공격자 인프라에서의 접근 → 즉시 CRITICAL
- 목적지 IP 일치: 내부 호스트가 악성 IP로 순수 SYN을 전송할 때만 탐지 (응답 패킷·SYN-ACK는 오탐 방지를 위해 무시)
- DNS 도메인 일치: 피싱/악성 도메인 조회 → CRITICAL

#### C2 Beaconing (`c2_beaconing`)
> IAT(패킷 도착 간격) 분산 계수 분석으로 C2 비콘 콜백 패턴을 탐지한다.

**탐지 원리**: C2 악성코드는 일정한 주기로 서버에 콜백하므로 동일 `(src_ip, dst_ip, dst_port)` 쌍의 패킷 도착 간격(IAT) 분산 계수(CV = 표준편차/평균)가 매우 낮다(≈0). 정상적인 인간 발생 트래픽은 불규칙하여 CV가 높다. `min_connections`개 이상의 샘플이 누적되고, IAT 평균이 `min_interval`~`max_interval` 범위 안에 있으며, CV가 `cv_threshold` 이하이면 비콘으로 판정한다.

| 파라미터 | 기본값 | 설명 |
|----------|--------|------|
| `cv_threshold` | 0.3 | IAT 분산 계수 임계값 (낮을수록 엄격) |
| `min_connections` | 8 | 비콘 판정에 필요한 최소 IAT 샘플 수 |
| `min_interval` | 5.0s | 비콘으로 판정하는 최소 평균 IAT |
| `max_interval` | 3600.0s | 비콘으로 판정하는 최대 평균 IAT |

---

### 세그먼트 정책 및 다크 스페이스

#### Segment Violation (`segment_violation`)
> 설정된 네트워크 세그먼트 격리 정책을 위반하는 트래픽을 탐지한다.

**탐지 원리**: `allowed_flows` 목록에 허용된 네트워크 세그먼트 간 플로우를 정의한다. 목록에 없는 세그먼트 경계를 넘는 트래픽(src_net → dst_net, 포트, 프로토콜)이 감지되면 즉시 CRITICAL 알림을 발생시킨다. `allowed_flows`가 비어 있으면 탐지가 비활성화된다.

| 파라미터 | 기본값 | 설명 |
|----------|--------|------|
| `cooldown_seconds` | 300 | 동일 (src, dst) 쌍 알림 재발생 억제 시간 |
| `allowed_flows` | `[]` | 허용 플로우 목록 (src_net, dst_net, ports, proto 구성) |
...
#### Dark IP (`dark_ip`)
> 내부 네트워크에서 알려지지 않은 IP(다크 스페이스)로 향하는 트래픽을 탐지한다.

**탐지 원리**: `monitored_networks` CIDR 블록 안에서 실제로 통신한 적 없는 목적지 IP로 패킷이 전송되면 경고를 발생시킨다. `learn_seconds` 동안은 활성 호스트를 자동 학습하고 경고를 억제한다. 학습 기간 이후 새로 등장한 목적지 IP가 감시 대역에 속하면 다크 스페이스 접근으로 판정한다. `monitored_networks`가 비어 있으면 비활성화된다.

| 파라미터 | 기본값 | 설명 |
|----------|--------|------|
| `learn_seconds` | 300 | 시작 후 학습 기간 (경고 없이 활성 호스트 수집) |
| `cooldown_seconds` | 3600 | 동일 목적지 IP 알림 재발생 억제 시간 |
| `auto_learn` | `true` | 관찰된 소스 IP를 자동으로 알려진 호스트에 추가 |
| `monitored_networks` | `[]` | 감시 대상 내부 네트워크 CIDR 목록 |
| `known_hosts` | `[]` | 정적으로 알려진 내부 호스트 IP 목록 |
...
### NetFlow 기반 탐지 엔진 (SPAN 불필요)

> **활성화 조건**: `config/default.yaml`에서 `netflow.enabled: true` 설정 후 재시작.
> 라우터가 이 장비의 UDP 2055 포트로 NetFlow v5를 내보내야 한다.

#### Flow Port Scan (`flow_port_scan`)
> 라우터 NetFlow 기록에서 포트 스캔을 탐지한다.

**탐지 원리**: `(src_ip, dst_ip)` 쌍별로 슬라이딩 윈도우 내에 관찰된 고유 목적지 포트 수를 추적한다. 임계값(`threshold`) 초과 시 포트 스캔으로 탐지한다. 패킷 수준 `port_scan` 엔진과 달리 SPAN 없이도 동작하며, 라우터를 통과하는 모든 플로우를 분석한다.

| 파라미터 | 기본값 | 설명 |
|----------|--------|------|
| `window_seconds` | 60 | 고유 포트 집계 윈도우 |
| `threshold` | 15 | 고유 목적지 포트 수 임계값 |
| `cooldown_seconds` | 300 | 동일 알림 재발생 쿨다운 |

#### Flow Data Exfil (`flow_data_exfil`)
> 내부 호스트에서 외부로 대용량 데이터 전송(데이터 유출)을 탐지한다.

**탐지 원리**: RFC 1918 사설 IP에서 공인 IP로의 전송 바이트를 `(src_ip, dst_ip)` 쌍별로 슬라이딩 윈도우 내에 누적한다. 누적량이 `byte_threshold`를 초과하면 데이터 유출 의심으로 탐지한다. 중복 알림 억제는 AlertDispatcher의 rate limiter에 위임한다.

| 파라미터 | 기본값 | 설명 |
|----------|--------|------|
| `byte_threshold` | 104857600 (100MB) | 유출 의심 누적 바이트 임계값 |
| `window_seconds` | 3600 | 전송량 집계 윈도우 (초) |
...
| **Blocklist** | 커스텀 IP/도메인 차단 목록 관리, 위협 피드 통계 |
| **Whitelist** | 허용 목록 관리 — IP/MAC/도메인/IP Range 추가·삭제, 타입별 필터·검색. 화이트리스트 항목은 AI 분석 프롬프트에도 포함되어 오탐 판정에 반영됨 |
| **Engines** | 21개 탐지 엔진 활성화/비활성화 토글, 파라미터 실시간 편집 |
...
21개 탐지 엔진의 활성화/비활성화 토글과 파라미터 실시간 편집. 변경 사항은 재시작 없이 즉시 반영된다.
